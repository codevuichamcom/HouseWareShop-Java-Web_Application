...
<html lang="en">
    <head>
        ...
        <style>
            <c:if test="${match eq 1}">#m{color: yellow !important}</c:if>
            <c:if test="${random eq 1}">#r{color: yellow !important}</c:if>
            <c:if test="${price >= 0}">#p{color: yellow !important}</c:if>
            <c:if test="${news eq 1}">#n{color: yellow !important}</c:if>
            <c:if test="${free eq 1}">#f{color: yellow !important}</c:if>
        </style>
    </head>
    <body>
                ...
                <div class="col-md-10">
                    <h5 class="labelhead">
                        <div class="alert alert-secondary" role="alert">
                            <form id="frm2" onchange="submitForm()" action="sort-product" method="post">List of products
                                <input type="hidden" name="page" value="${page}">
                                <input type="hidden" id="match" name="match" value="${match}">
                                <input type="hidden" id="random" name="random" value="${random}">
                                <input type="hidden" id="news" name="news" value="${news}">
                                <input type="hidden" id="free" name="free" value="${free}">
                                <i id="m" class="fas fa-cubes mr-2 ml-4"></i></i><a onclick="<c:if test="${match ne 1}">document.getElementById('match').value=1</c:if><c:if test="${match eq 1}">document.getElementById('match').value=-1</c:if>" class="btn btn-outline-warning" active style="color: #ff0707"> Best Match</a>
                                <i id="r" class="fas fa-filter mr-2 ml-4"></i></i><a onclick="<c:if test="${random ne 1}">document.getElementById('random').value=1</c:if><c:if test="${random eq 1}">document.getElementById('random').value=-1</c:if>" class="btn btn-outline-primary">Random</a>
                                <i id="p" class="fas fa-dollar-sign mr-2 ml-4 act"></i>
                                <select name="price" style="font-size: 1rem">
                                    ...
                                    <i id="n" class="fas fa-sort-down mr-2 ml-4"></i><a onclick="<c:if test="${news ne 1}">document.getElementById('news').value=1</c:if><c:if test="${news eq 1}">document.getElementById('news').value=-1</c:if>" class="btn btn-outline-info">new</a>
                                    <i id="f" class="fas fa-bolt mr-2 ml-4 act"></i><a onclick="<c:if test="${free ne 1}">document.getElementById('free').value=1</c:if><c:if test="${free eq 1}">document.getElementById('free').value=-1</c:if>" class="btn btn-outline-success">free ship</a>
                                    <a onclick="document.getElementById('frm2').submit()" style="color: orange"><i class="fas fa-sort ml-5"></i></a>
                                </form>
                            </div>
